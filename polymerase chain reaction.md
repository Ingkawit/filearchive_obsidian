components
- DNA sample
- forward primer and reverse primer
- nucleotide (dNTP)
- Taq polymerase - thermostable
- mix buffer - magnesium
- thermal cycler

process:
1. denaturing (95ºC)
	- strands separate
2. annealing (50-60ºC)
	- forward and backward primer bind to each strand
3. extension (72ºC)
	- extension from primers, creating new strand

repeat n cycles -> 2^n molecules 

4.  [[gel electrophoresis]]
	- screening
	- detect deletion, insertion


### RT-PCR
difference from regular PCR
- RNA sample
- reverse transcriptase - turn RNA into cDNA
regular PCR process 

#### nested PCR
- using PCR product as template for a new PCR
#### multiplex PCR
- more than 1 pair of primer in single tube
#### RFLP
restriction fragment length polymorphism
- detect point mutation
#### real-time PCR (quantitative PCR)
- track DNA with fluorescent molecule eg. SYBR green, TaqMan probe
- cycle threshold (Ct) - how many cycles to produce more DNA than threshold
- lower Ct -> high amount of start DNA
- can measure exact amount of DNA

![[FullSubwayMap_V1023_Web.pdf]]